...
fun getDetailDataById(id: String) {
    ...
    val director = doc.select("li[data-testid='title-pc-principal-credit']:has(span:contains(Director)) a").text()

    val writers = doc.select("li[data-testid='title-pc-principal-credit']:has(a:contains(Writers)) a.ipc-metadata-list-item__list-content-item").eachText()

    val stars = doc.select("li[data-testid='title-pc-principal-credit']:has(a:contains(Stars)) a.ipc-metadata-list-item__list-content-item").eachText()

    val userReviews = doc.select("ul[data-testid='reviewContent-all-reviews'] .three-Elements .score").text()
